...
function GalleryItem({recipeCard, i}:any){
    return(
        ...
        <div className='recipe-card'>
          <Card.Body>
            ...
            <Card.Text style={{ textAlign: "left", fontWeight: "bold", color: "#b5460f" }} color="text.secondary">{recipeCard.author}</Card.Text>
            <Card.Text style={{ textAlign: "justify" }}>{recipeCard.description}</Card.Text>
            <Button style={{ backgroundColor: "#b5460f", border: "1px solid grey" }} href={`/${recipeCard.recipe_id}`}>Read more</Button>
          </Card.Body>
        </div>
      </Card>
    )
